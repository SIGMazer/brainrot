...
short evaluate_expression_short(ASTNode *node)
{
    ...
    case NODE_OPERATION:
    {
        ...
        {
            short left = evaluate_expression_int(node->data.op.left);
            short right = evaluate_expression_int(node->data.op.right);
        ...
        short result_type = get_expression_type(node);
        void *result = handle_binary_operation(node, result_type);
        short result_int = 0;
        result_int = (result_type == VAR_SHORT)
                         ? *(short *)result
                     : (result_type == VAR_FLOAT)
                         ? (short)(*(float *)result)
                         : (short)(*(double *)result);
